...
(
    ...
    UNION ALL
    (
        -- 5. Comment on media in a friend's public post (by anyone)
        SELECT DISTINCT
            p.id as post_id_for_activity,
            p.content as post_content_preview,
            CONCAT_WS(' ', pa.first_name, pa.middle_name, pa.last_name) as post_author_name,
            pa.id as post_author_id,
            'comment_on_friend_post' as activity_type, -- Using same type as comment on post for now
            CONCAT_WS(' ', commenter.first_name, commenter.middle_name, commenter.last_name) as actor_name,
            commenter.profile_pic as actor_profile_pic, commenter.gender as actor_gender,
            mc.created_at as activity_time,
            mc.id as event_id,
            NULL as reaction_type,
            commenter.id as actor_user_id,
            pa.id as target_friend_user_id,
            CONCAT_WS(' ', pa.first_name, pa.middle_name, pa.last_name) as target_friend_name,
            NULL as other_friend_name, NULL as other_friend_user_id,
            NULL as testimonial_id, NULL as testimonial_content, NULL as testimonial_rating,
            NULL as actual_writer_name, NULL as actual_writer_id,
            NULL as activity_id_social, NULL as extra_info,
            mc.media_id as media_id -- Specific to this type
        FROM media_comments mc
        JOIN users commenter ON mc.user_id = commenter.id
        JOIN post_media pm ON mc.media_id = pm.media_id
        JOIN posts p ON pm.post_id = p.id
        JOIN users pa ON p.user_id = pa.id
        WHERE p.visibility = 'public'
        AND p.user_id IN (
            SELECT CASE WHEN sender_id = :user_id17 THEN receiver_id ELSE sender_id END
    ...
    UNION ALL
    (
        -- 6. Reaction to media in a friend's public post (by anyone)
        SELECT DISTINCT
            p.id as post_id_for_activity,
            p.content as post_content_preview,
            CONCAT_WS(' ', pa.first_name, pa.middle_name, pa.last_name) as post_author_name,
            pa.id as post_author_id,
            'reaction_to_friend_post' as activity_type, -- Using same type as reaction on post for now
            ...
            NULL as actual_writer_name, NULL as actual_writer_id,
            NULL as activity_id_social, NULL as extra_info,
            mr.media_id as media_id -- Specific to this type
        ...
        JOIN post_media pm ON mr.media_id = pm.media_id
        JOIN posts p ON pm.post_id = p.id
        JOIN users pa ON p.user_id = pa.id
        WHERE p.visibility = 'public'
        AND p.user_id IN (
    ...
    $activity_sources = [
        'post_related' => $post_related_activities, // Renamed $friend_activities
        'social' => $social_activities,
        'testimonial' => $testimonial_activities
    ];

    foreach ($activity_sources as $source_type => $source_array) {
        foreach ($source_array as $activity) {
            $unique_event_key = $activity['activity_type'] . '_';
            if (isset($activity['event_id'])) { // For comments, reactions, testimonials
                $unique_event_key .= $activity['event_id'];
            } elseif (isset($activity['activity_id_social'])) { // For social (friend requests)
                $unique_event_key .= $activity['activity_id_social'];
            } else {
                // Fallback for friend_connection which might not have a single unique event id from fr.id if multiple people become friends at once in theory
                // Using a composite key of the involved users for friend_connection
                 if ($activity['activity_type'] === 'friend_connection') {
                    $user1 = min($activity['actor_user_id'], $activity['other_friend_user_id']);
                    $user2 = max($activity['actor_user_id'], $activity['other_friend_user_id']);
                    $unique_event_key .= $user1 . '_' . $user2;
                } else {
                    // If no specific event_id, create a hash or less reliable key (might still have duplicates)
                    $unique_event_key .= md5(json_encode($activity)); // Less ideal, but a fallback
                }
            }
            ...
            $item = [
                ...
                'actor_name' => $activity['actor_name'] ?? 'Unknown User',
            ...
            if (strpos($activity['activity_type'], 'post') !== false) { // comment, reaction_on_friend_post, etc.
                $item['post_id_for_activity'] = $activity['post_id_for_activity'] ?? null;
                $item['post_content_preview'] = $activity['post_content_preview'] ?? null;
                $item['post_author_name'] = $activity['post_author_name'] ?? 'A post';
                $item['post_author_id'] = $activity['post_author_id'] ?? null;
                $item['post_id'] = $item['post_id_for_activity']; // For JS compatibility with viewPost
                $item['post_author'] = $item['post_author_name']; // For JS comment type compatibility

                if ($activity['activity_type'] === 'comment' || $activity['activity_type'] === 'comment_on_friend_post') {
                    $item['comment_id'] = $activity['event_id'] ?? null; // from c.id as event_id
                }
                if ($activity['activity_type'] === 'reaction_on_friend_post' || $activity['activity_type'] === 'reaction_to_friend_post') {
                    $item['reaction_type'] = $activity['reaction_type'] ?? null;
                     // $item['reaction_id'] = $activity['event_id']; // from pr.reaction_id as event_id
                }
                if ($activity['activity_type'] === 'comment_on_friend_post' || $activity['activity_type'] === 'reaction_to_friend_post') {
                    $item['target_friend_user_id'] = $activity['target_friend_user_id'] ?? null;
                    $item['target_friend_name'] = $activity['target_friend_name'] ?? 'a friend';
                }
                 // For original 'comment' and 'reaction_on_friend_post' JS compatibility
            ...
            } elseif (strpos($activity['activity_type'], 'friend') !== false) { // friend_request, friend_connection
                $item['other_friend_name'] = $activity['other_friend_name'] ?? null;
                $item['other_friend_user_id'] = $activity['other_friend_user_id'] ?? null;
                $item['activity_id'] = $activity['activity_id_social'] ?? null; // from fr.id as activity_id_social
                $item['extra_info'] = $activity['extra_info'] ?? null;
                 // For JS compatibility for friend_request/friend_connection if it expects friend_name etc.
                $item['friend_name'] = $item['actor_name'];
                $item['friend_user_id'] = $item['actor_user_id'];


            } elseif (strpos($activity['activity_type'], 'testimonial') !== false) {
                $item['testimonial_id'] = $activity['event_id'] ?? null; // from t.testimonial_id as event_id
                $item['content'] = $activity['testimonial_content'] ?? null;
                $item['rating'] = $activity['testimonial_rating'] ?? null;

                if ($activity['activity_type'] === 'testimonial_written') {
                    $item['writer_name'] = $activity['actor_name'];
                    $item['writer_id'] = $activity['actor_user_id'];
                    $item['recipient_name'] = $activity['target_friend_name'] ?? 'Someone';
                    $item['recipient_id'] = $activity['target_friend_user_id'] ?? null;
                } elseif ($activity['activity_type'] === 'testimonial_received') {
                    $item['writer_name'] = $activity['actual_writer_name'] ?? 'Someone';
                    $item['writer_id'] = $activity['actual_writer_id'] ?? null;
                    $item['recipient_name'] = $activity['actor_name'];

...
try {
    ...
    if ($tableExists) {
        // Full query with user_conversation_settings
        $sql = "
            SELECT DISTINCT 
                m.thread_id as id,
        ...
        // Simplified query without user_conversation_settings
        $sql = "
            SELECT DISTINCT 
                m.thread_id as id,
                u.id as participant_id,
                ...
                FALSE as is_muted,
                FALSE as is_archived_for_user,
                FALSE as is_deleted_for_user
            FROM messages m
            JOIN users u ON (
                CASE 
                    WHEN m.sender_id = ? THEN m.receiver_id = u.id
    ...
    try {
        ...
        header('Content-Type: application/json');
        // Return empty array instead of error object to prevent JavaScript errors
        echo json_encode([]);
    }
} catch (Exception $e) {
    error_log("Error in chat_threads.php: " . $e->getMessage());

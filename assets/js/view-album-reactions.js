...
window.SimpleReactionSystem = {
  ...
  handleClick: function(event) {
    ...
    if (reactionOption && reactionOption.parentElement.closest('#simple-reaction-picker')) {
      ...
      console.log('Reaction clicked:', mediaId, reactionId, reactionName);
  ...
  async sendReactionToServer(mediaId, reactionTypeId, toggleOff = false) {
    try {
      ...
      // Send the actual request without the check that's causing 400 errors
      const response = await fetch(`api/media_reaction.php?_=${timestamp}`, {
        method: 'POST',
        headers: {
...
window.SimpleReactionSystem = window.SimpleReactionSystem || {};
